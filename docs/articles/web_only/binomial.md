...
$N = 216$ patients. The simplest way to specify a fixed design with
**BATSS** is to set `interim = NA`, which instructs the function to
perform a single look at the maximum sample size `N`. The `RAR` argument
is set to `NULL` as response-adaptive randomisation is not applicable in
a fixed design.

#### Scenario 1 (fixed)

Same as the adaptive Scenario 1 above, except `interim = NA` (fixed
design, single look at $N = 216$), `delta.eff` and `delta.fut` are set
to scalar values `0` and `log(1.5)` respectively.
