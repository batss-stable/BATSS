...
changes to the default prior for the fixed effects:
...
default prior used in `BATSS` are listed in Table 1 (see Simpson et al.
(2017) for an in depth description of complexity priors). Detailed
information can be viewed by typing
...
```
    ...
    ## function (x) 
    ## log(x)
    ## <bytecode: 0x131635a88>
    ## <environment: 0x131631e78>
    ## attr(,"inla.read.only")
    ## [1] TRUE
    ## 
